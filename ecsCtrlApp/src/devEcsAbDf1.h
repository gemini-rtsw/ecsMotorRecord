...
#ifndef DEVECSABDF1_H
#define DEVECSABDF1_H
...
/* Module internal definitions for ecsMotor device support */

/* Background task scan rate in scans/second */
#define ECS_SCAN_RATE		5
 ...
 */
typedef struct absWordIO {
  const ELLNODE		node;			/* private to drvAbDf1 */
  epicsUInt16           stationNumber; 		/* supplied by app */
  epicsUInt16           wordAddr; 		/* supplied by app */
  //drvAbDf1LinkId     	linkId;			/* supplied by app */
  drvAbDf1Parm    	**linkId;		/* supplied by app */
  void			(*pCB)
     (struct absWordIO *pIO, long status);	/* supplied by app */

  void			*pPriv;			/* supplied by app */
  IOSCANPVT		ioScanPvt;		/* used by device support */
  const drvAbDf1CacheId cacheId;		/* private to drvAbDf1 */
  const unsigned char	ioInProg;		/* private to drvAbDf1 */
  unsigned char		oneIOReq;		/* used by device support */
} absWordIO; 

// typedef void absIODoneCallBack(absWordIO *pIO, long status);
